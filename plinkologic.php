...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const multipliers = [16, 9, 4, 2, 1.5, 1, 0.5, 0.2, 0.2, 0.2, 0.5, 1, 1.5, 2, 4, 9, 16];
  const binsCount = multipliers.length;

  const binSpacing = 4;
  const binBorderRadius = 10;
  ...
  async function updateChipsServer(delta) {
    try {
      ...
      if (json.success) {
        ...
        statusMessage.textContent = "Fehler: " + (json.error || "Unbekannter Fehler");
      }
    } catch (e) {
      statusMessage.className = "error";
  ...
  function startBall(bet) {
    ...
    if (isNaN(bet) || bet < 1) {
      statusMessage.className = "error";
      statusMessage.textContent = "Bitte gültigen Einsatz angeben!";
      return;
    }

    if (bet > currentChips) {
      statusMessage.className = "error";
      statusMessage.textContent = "Nicht genug Chips!";
      return;
    }

    statusMessage.className = "info";
    statusMessage.textContent = "Ball rollt...";
  ...
  function animate() {
    ...
    if (nextRow < ROWS && ball.y >= (60 + nextRow * SPACING_Y - BALL_RADIUS)) {
      let startIdx = (nextRow * (nextRow + 1)) / 2;
      let endIdx = ((nextRow + 1) * (nextRow + 2)) / 2;
      let pegsRow = pegs.slice(startIdx, endIdx);
      let closest = pegsRow.reduce((prev, curr) =>
        Math.abs(curr.x - ball.x) < Math.abs(prev.x - ball.x) ? curr : prev
      );
    ...
    if (!ball.falling) {
      ...
      if (ball.y >= canvasHeight - 40 - BALL_RADIUS) {
        ...
        const multiplier = bins[index].multiplier;
        const winAmount = Math.floor(ball.bet * multiplier);

        updateChipsServer(winAmount);
        updateChipsDisplay(currentChips + winAmount);
        currentChips += winAmount;

        statusMessage.className = "success";
        statusMessage.textContent = `Gewonnen: ${winAmount} Chips (x${multiplier})!`;
